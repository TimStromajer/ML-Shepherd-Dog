        ...
        if (rotationCalcTimerRun > rotationCalcRun)
        {
            ...
            foreach (GameObject s in dogList)
            {
                if (Vector3.Distance(s.transform.position, transform.position) < GM.strombom.rs)
                {
                    ...
                    if (Vector3.Distance(s.transform.position, transform.position) <= 4)
                    {
                        dogVeryClose = 1.2f;

                        // if dog very close -> add more impact
                        dogVector += 2 * Vector3.Normalize(transform.position - s.transform.position);
                    }
                }
            }
            ...
            Vector3 final = transform.forward * GM.strombom.h + dogVector * GM.strombom.ro_s + sheepVector * GM.strombom.ro_a + gcmVector * GM.strombom.c + obstacleVect * GM.obstacleVectStrength;
            final = Vector3.Normalize(final);

            if (info)
            {
                Debug.DrawLine(transform.position, transform.position + 5 * dogVector, Color.green);
                Debug.DrawLine(transform.position, transform.position + 5 * sheepVector, Color.blue);
                Debug.DrawLine(transform.position, transform.position + 5 * gcmVector, Color.white);

                Debug.DrawLine(transform.position, transform.position + 5 * final, Color.red);
            }

            desiredRotationVec = final;
            if (!final.Equals(new Vector3(0, 0, 0)))
            {
